...
public class CSVWriterParamAnalysis {
    ...
    public CSVWriterParamAnalysis(String path,double l){
        this.filePath = path;
        try {
            ...
            while(csvNotInitialized && counter < 50) {
                ...
                try {
                    ...
                    if (fileEmpty) {
                        FileWriter writer = new FileWriter(path, false);
                        csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT.withHeader("Function","AlgorithmnName","status"));
                        csvNotInitialized=false;
                    } else {
                        FileWriter writer = new FileWriter(path, true);
                        csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT);
    ...
    public void addRecord(String function ,String algorithmnName,String status) throws IOException {
        if(csvPrinter != null) {
            csvPrinter.printRecord(function,algorithmnName, status);
        }
    }
